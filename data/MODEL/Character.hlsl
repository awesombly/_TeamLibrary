...
VS_OUTPUT VS(PNCT5_VS_INPUT input)//,uniform bool bHalfVector )
{
	...
	//{
		...
		output.col.xyz = input.col.xyz * max(cb_useLight + 0.15f, dot(vLightDir, output.nor.xyz) + 0.45f);
		//output.col.xy *= 0.85f;
...
PBUFFER_OUTPUT PS(VS_OUTPUT input) : SV_Target
{
	...
	if (cb_useEnviMap)
	{
		...
		//{
			...
			//// 굴절, 반사 보간
			//output.color0.xyz = lerp(output.color0.xyz, lerp(refractColor.xyz, reflectColor.xyz, fresnel * 7.0f), 0.3f)/* * input.col*/;
