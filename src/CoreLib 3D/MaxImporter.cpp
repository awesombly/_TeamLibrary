...
map<wstring, MaxImporter::EAseTag> MaxImporter::m_AseTag;
...
MaxImporter::MaxImporter() noexcept
{
	...
	std::call_once(flag, [&]() {
		...
		m_AseTag[L"Material"]	= EAseTag::MaterialList;
		m_AseTag[L"Mesh"]		= EAseTag::Mesh;
	});
}
...
bool MaxImporter::LoadMaterialList(const int& parentCnt) noexcept
{
	...
	for(int iParent = 0; iParent < parentCnt; ++iParent)
	{
		...
		*(++m_curLineIter) >> pMaterial->MaterialIndex >> childCnt >> textureCnt;
		...
		{
			*m_curLineIter >> m_subString;
			pMaterial->pTexture = DxManager::GetInstance().GetTexture(m_subString, true);
		}
		if (childCnt <= 0)
			continue;
		...
		for (int iChild = 0; iChild < childCnt; ++iChild)
		{
			...
			*(++m_curLineIter) >> pChild->MaterialIndex >> tempInt >> textureCnt;
			...
			{
				*m_curLineIter >> m_subString;
				pChild->pTexture = DxManager::GetInstance().GetTexture(m_subString, true);
				if (pChild->pTexture == nullptr)
				{
					pChild->pTexture = DxManager::GetInstance().GetTexture(L"Model/" + m_subString);
					if (pChild->pTexture == nullptr)
					{
						pChild->pTexture = DxManager::GetInstance().GetTexture(L"Effect3d/" + m_subString);
						if (pChild->pTexture == nullptr)
						{
...
bool MaxImporter::LoadObject(const int& objCnt) noexcept
{
	...
	map<wstring, wstring> parentList;
		...
		case EClassType::Geom:
		{
			...
			MaterialData* pMaterial = &m_MaterialList[matRef];
			{
				...
				for (auto& iter : pMaterial->SubMaterials)
				{
					...
					{
						ErrorMessage(L""s + __FUNCTIONW__ + L" -> " + m_subString.data() + L", " + to_wstring(iter.MaterialIndex) + L" : Material nullptr!");
