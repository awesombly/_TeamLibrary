	...
	for (int i = 0; i < 4; ++i)
	{
		if (VectorLengthSq(m_CarpetPos[i] - GetWorldPosition()) <= 5000.0f)
		{
			...
			case ECarpet::Smithy:
			{
				...
				pUIManager->m_pSmithyInfo1Weapon->SetString(L"Level " + to_wstring(m_upgradeArmor) + L" °Ê Level " + to_wstring(m_upgradeArmor + 1));
				...
				pUIManager->m_pSmithyInfo1Weapon->SetString(L"Level " + to_wstring(m_upgradeAcce1) + L" °Ê Level " + to_wstring(m_upgradeAcce1 + 1));
				...
				pUIManager->m_pSmithyInfo1Weapon->SetString(L"Level " + to_wstring(m_upgradeAcce1) + L" °Ê Level " + to_wstring(m_upgradeAcce2 + 1));
				pUIManager->m_pSmithyInfo2Acce2->SetString(L"All Stat " + to_wstring(m_upgradeAcce2) + L" °Ê " + to_wstring(m_upgradeAcce2));
			}	break;
			case ECarpet::Church:
			{
