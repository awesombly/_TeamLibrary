	...
	for (int i = 0; i < 4; ++i)
	{
		if (VectorLengthSq(m_CarpetPos[i] - GetWorldPosition()) <= 5000.0f)
		{
			...
			case ECarpet::Smithy:
			{
				...
				pUIManager->m_pSmithyInfo2Weapon->SetString(L"Dmg " + to_wstring((m_upgradeWeapon) * 15) + L"% °Ê " + to_wstring((m_upgradeWeapon + 1) * 15) + L"%");

				pUIManager->m_pSmithyInfo1Armor->SetString(L"Level " + to_wstring(m_upgradeArmor) + L" °Ê Level " + to_wstring(m_upgradeArmor + 1));
				pUIManager->m_pSmithyInfo2Armor->SetString(L"Arm " + to_wstring((1.0f - (5.0f / (5.0f + m_defencePoint + m_upgradeArmor))) * 100.0f).substr(0, 4) + L"% °Ê " + to_wstring((1.0f - (5.0f / (5.0f + m_defencePoint + m_upgradeArmor + 1))) * 100.0f).substr(0, 4) + L"%"); 

				pUIManager->m_pSmithyInfo1Acce1->SetString(L"Level " + to_wstring(m_upgradeAcce1) + L" °Ê Level " + to_wstring(m_upgradeAcce1 + 1));
				pUIManager->m_pSmithyInfo2Acce1->SetString(L"Cool " + to_wstring((1.0f - (5.0f / (5.0f + m_upgradeAcce1))) * 100.0f).substr(0, 4) + L"% °Ê " + to_wstring((1.0f - (5.0f / (5.0f + m_upgradeAcce1 + 1))) * 100.0f).substr(0, 4) + L"%");

				pUIManager->m_pSmithyInfo1Acce2->SetString(L"Level " + to_wstring(m_upgradeAcce1) + L" °Ê Level " + to_wstring(m_upgradeAcce2 + 1));
			...
			case ECarpet::Tower:
			{
				...
				if (PacketManager::Get().TowerLevel == 4)
					pUIManager->m_pTowerText2->SetString(L"∆¯≈∫ √ﬂ∞°");
				else
