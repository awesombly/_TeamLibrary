...
bool GameMap::Init()											noexcept
{
	...
	pCollider->m_pivot = Vector3::Up * 35.0f;
...
#pragma region MyTower
	...
	//╩С
	m_tower0.SetPlayerCharacter(L"MAP_Tower", 250.0f, 0.0f, 900.0f);	//280;
	m_tower0.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));;
	m_tower0.m_objType = EObjType::AObject;
	ObjectManager::Get().PushObject(&m_tower0);

	m_tower1.SetPlayerCharacter(L"MAP_Tower", -250, 0.0f, 900.0f);	//280
	m_tower1.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));;
	m_tower1.m_objType = EObjType::AObject;
	ObjectManager::Get().PushObject(&m_tower1);


	//©Л
	m_tower2.SetPlayerCharacter(L"MAP_Tower", 900.0f, 0.0f, 250);	//280;
	m_tower2.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));
	m_tower2.SetRotationY(PI*0.5f);
	m_tower2.m_objType = EObjType::AObject;
	ObjectManager::Get().PushObject(&m_tower2);

	m_tower3.SetPlayerCharacter(L"MAP_Tower", 900.0f, 0.0f, -250);	//280;
	m_tower3.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));
	m_tower3.SetRotationY(PI*0.5f);
	m_tower3.m_objType = EObjType::AObject;
	ObjectManager::Get().PushObject(&m_tower3);

	//го
	m_tower4.SetPlayerCharacter(L"MAP_Tower", 250, 0.0f, -900.0f);	//280;
	m_tower4.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));
	m_tower4.m_objType = EObjType::AObject;
	ObjectManager::Get().PushObject(&m_tower4);

	m_tower5.SetPlayerCharacter(L"MAP_Tower", -250, 0.0f, -900.0f);	//280;
	m_tower5.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));
	m_tower5.m_objType = EObjType::AObject;
	ObjectManager::Get().PushObject(&m_tower5);

	//аб
	m_tower6.SetPlayerCharacter(L"MAP_Tower", -900.0f, 0.0f, 250);	//280;
	m_tower6.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));;
	m_tower6.SetRotationY(PI*0.5f);
	m_tower6.m_objType = EObjType::AObject;
	ObjectManager::Get().PushObject(&m_tower6);

	m_tower7.SetPlayerCharacter(L"MAP_Tower", -900.0f, 0.0f, -250);	//280;
	m_tower7.SetScale(D3DXVECTOR3(0.8f, 0.8f, 0.8f));;
	m_tower7.SetRotationY(PI*0.5f);
